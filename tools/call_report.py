...
def last_networked_rf_call(
    ...
    for start in sorted(rf_voice_events, key=lambda item: item.ts, reverse=True):
        ...
        if not end:
            continue
        network_hosts = network_voice_hosts_for_call(start, end, net_voice_events)
        if network_hosts:
...
def build_report(path: Path, include_log: bool = True) -> str:
    ...
    for event_type, event in iter_events(path):
        if event_type == "net_voice":
            last_net_voice[event.host] = event
    ...
    final_networked_call = last_networked_rf_call(rf_voice_events, rf_end_events, net_voice_events)
    if final_networked_call:
        ...
        final_voice_ts = max(event.ts for event in last_net_voice.values())
        active_hosts = {
            event.host
            for event in last_net_voice.values()
            if event.ts >= final_voice_ts - timedelta(milliseconds=FINAL_CALL_WINDOW_MS)
        }

    earlier_rows = [
        [event.host, format_ts(event.ts)]
        for event in sorted(last_net_voice.values(), key=lambda item: (item.ts, item.host))
